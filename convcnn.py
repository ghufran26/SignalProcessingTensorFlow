...
# LSTM
############################
xtrain   = xtrain.reshape(xtrain.shape[0],timesteps, 501, 1).astype('float32')
xval     = xval.reshape(xval.shape[0],timesteps, 501, 1).astype('float32')
xtest    = xtest.reshape(xtest.shape[0],timesteps, 501, 1).astype('float32')
...
class LSTM(Model):
    ...
    def __init__(self):
        ...
        self.cnn  = layers.TimeDistributed(layers.Conv1D(filters=6, kernel_size=48,strides=32, activation='relu', padding='same'),batch_input_shape=(None,timesteps, 501, 1))
